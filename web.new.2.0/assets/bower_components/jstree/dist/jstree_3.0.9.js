		...
		edit : function (obj, default_text) {
			...
			var rtl = this._data.core.rtl,
				...
				*/
				...
				h2 = $("<"+"input />", {
						"value" : t,
						"class": "jstree-rename-input form-control form-white",
						// "size" : t.length,
						"css" : {
							"padding" : "0",
			...
			var hh = Math.min(h1.text("pW" + h2[0].value).width(), w);
			
			h2.css(fn).width(hh)[0].select();
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		this.bind = function () {
			...
			if(this.settings.checkbox.three_state) {
			    this.settings.checkbox.cascade = 'undetermined+up';
		    }
			
			if (this.settings.checkbox.checkChildren) {
		        this.settings.checkbox.cascade += '+down';
			}

			if (!this.settings.checkbox.checkParent) {
					...
					.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', $.proxy(function (e, data) {
							...
							if (s.indexOf('up') !== -1) {
								...
								while(par && par.id !== '#') {
									...
									var array = this._data[t ? 'core' : 'checkbox'].selected;
									if (!RegExp("\\b" + par.id + "\\b").test(array)) {
