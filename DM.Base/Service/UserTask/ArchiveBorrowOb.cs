...
{
    public class ArchiveBorrowOb : IObservation
    {
        [Dependency("System2")]
        ...
        {
           var pInfo = _IObjectProcessService.Get(pi.ID);

            _IBorrowService.ApproveDone(pInfo.ObjectID);
        }
        ...
        public virtual async Task<bool> TaskTurnCall(BPMObArg arg)
        {
            var userTask = new UserTaskEntity()
            {
                ...
                Source = (int)UserTaskSource.档案借阅,
                Status = (int)UserTaskStatus.下达,
                Type = 1,
